...
public extension FlappyAPI {
    final class User: Codable {
        ...
        public init(id: UUID? = UUID(), name: String, score: Int? = 0, deaths: Int? = 0, passwordHash: String, jailbroken: Bool? = false, hasHackedTools: Bool? = false, ranInEmulator: Bool? = false, hasModifiedScore: Bool? = false, isBanned: Bool? = false, banReason: String? = nil, admin: Bool? = false, owner: Bool? = false) {
            self.id = id
            self.name = name
            self.score = score
            self.deaths = deaths
